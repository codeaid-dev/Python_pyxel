    ...
    def update(self):
        ...
        if self.is_title:
            ...
            if not self.goal:
                ...
                if pyxel.btn(pyxel.KEY_LEFT) and x-1 > -1 and self.maze[y][x-1] == 0:
                    self.player.x -= 1
                    self.player.dir = 0
                    self.player.status = not self.player.status
                if pyxel.btn(pyxel.KEY_RIGHT) and x+1 < self.mazew and self.maze[y][x+1] == 0:
                    self.player.x += 1
                    self.player.dir = 1
                    self.player.status = not self.player.status
                if pyxel.btn(pyxel.KEY_UP) and y-1 > -1 and self.maze[y-1][x] == 0:
                    self.player.y -= 1
                    self.player.dir = 2
                    self.player.status = not self.player.status
                if pyxel.btn(pyxel.KEY_DOWN) and y+1 < self.mazeh and self.maze[y+1][x] == 0:
                    self.player.y += 1
                    self.player.dir = 3
                    self.player.status = not self.player.status

                if self.goal_x == self.player.x and self.goal_y == self.player.y:
                    self.goal = True
